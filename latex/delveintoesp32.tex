...
Only installation instructions for Ubuntu Linux is provided in this book as an illustrative example. Instructions for other operating systems can be found by searching on-line for ''Espressif ESP32 installations instructions``.
Even if your computer runs another operating system than Ubuntu Linux so is it a good idea to glance through the instructions in the following sections since there are some general useful information that is valid for all installations regardless of the underlying operating system.

\subsection{General Software Packages Installation}
There are as discussed a number of software packages that are needed to be able to program an ESP32. Make sure that all of these are available by opening a terminal window and execute the following command. 
...
Expected result is that the apt-get utility that is a package management command line program, that is used to work with Ubuntu's Advanced Packaging Tool library, should perform installation of some new software packages.
...
Open a terminal and move to a location where you want to store the code to be downloaded. You can for example create a folder called esp that will hold various sub folders for the purpose of storing ESP-IDF as well as your own projects for the ESP32.
...
\subsection{ESP-IDF Environment Path Setup}
...
TODO add some instructions on installing the tool chain
